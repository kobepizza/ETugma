...
use App\Events\SendTutorMessage;
...
class TutorMessagingController extends Controller
{
   
    public function index()
    {
        return view('tutor.tutormessage');
    }

    public function loadContacts()
    {
        $TutorMainId = Session('tutorMain')->tutor->id;
        $LoggedUserId = session('loginId');

        $tutorIds = TutorMain::where('tutor_id',$TutorMainId)->get()->pluck('id');
        ...
        //dd($receivers);
        $parent = UserProfile::whereIn('id',  $receivers)
    ...
    public function countMessage()
    {
        ...
        $activeTutorSessions = TutorSession::with('guardianMain.guardian', 'tutorMain.tutor')
            ...
            ->whereHas('tutorMain.tutor', function ($query) use ($LoggedUserId) {
                $query->where('user_profile_id', $LoggedUserId);
            })
            ->get();

        // Pluck user_profile_ids of tutors, ensuring tutorMain and tutor exist
        $parentIds = $activeTutorSessions->map(function ($session) {
            return optional($session->guardianMain->guardian)->user_profile_id; // Safe null handling
        })->filter()->all(); // Filter out null values

        // Count chats where the sender is the tutor and the receiver is the logged-in user
        $chatCount = Chat::whereIn('sender_id', $parentIds)
    ...
    public function fetchMessagesFromParent(Request $request)
    {
    ...
    public function sendMessageToParent(Request $request)
    {
        ...
        Broadcast(new SendTutorMessage($chat, $receiverId, $senderId))->toOthers();
