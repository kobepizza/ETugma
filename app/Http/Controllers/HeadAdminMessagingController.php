...
use App\Events\SendHeadAdminMessage;
...
class HeadAdminMessagingController extends Controller
{
    public function index()
    {
        return view('head_admin.headadminmessage');
    }
            ...
            ->with(['userSender' => function ($query) use ($admins, $LoggedUserId) {
                $query->where(function ($q) use ($admins, $LoggedUserId) {
                    $q->whereIn('sender_id', $admins)
                        ->orWhere('receiver_id', $LoggedUserId);
    ...
    public function sendMessageToAdmins(Request $request)
    {
        ...
        Broadcast(new SendHeadAdminMessage($chat, $receiverId, $senderId))->toOthers();
