                    ...
                    <div class="card shadow rounded-4 mt-2 px-2" id="msg">
                        ...
                        <div class="reply-container py-3 d-flex align-items-center gap-1">
                            ...
                            <form class="p-0 mb-0" action="" method="POST" id="messageForm">
                                ...
                                <input type="hidden" value="" name="receiver_id" id="parentID">
                                <input type="hidden" value="{{session('loginId')}}" name="tutor_id" id="tutor_id">
...
<script>
    $(document).ready(function() {
        ...
        var activeContact;
        //console.log('token:', token, 'tutorID:', tutorId);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, contact) {
                        var html = '';
                        html += '<button class="list-group-item list-group-item-action contact-item" aria-current="true" type="button" parent-id="' + contact.id + '">';
                        html += '<input type="hidden" class="parent_id" value="' + contact.id + '">';
                        ...
                        html += '</div>';
                        html += '<div class="w-75 overflow-hidden" data-parent-id="' + contact.id + '">';
                        ...
                        $.each(contact.user_sender, function(index, chats) {
                            ...
                            html += '</div>';
                            html += '<div class="hstack align-items-center justify-content-between w-25" data-parent2-id="' + contact.id + '">';
        ...
        $('#contactList').on('click', '.contact-item', function() {
            activeContact = $(this).attr('parent-id');
            ...
            let receiverId = $(this).find('.parent_id').val();

            //console.log('chatID:',recentChat);

            $('#parentID').val(receiverId);
            ...
            let channelName = 'tutor-' + tutorId + '-parent-' + receiverId + '-chat';
           ...
           // console.log('Subscribed to:', channelName);
            ...
            currentChannel.bind('tutor-parent-chat', function(data) {
               // console.log('Message received:', data);
                ...
                function updateChatMessage(data, createdAt) {
                    var Contact = $('#contactList').find('[data-parent-id="' + sender_Id + '"]');
                    var chatMessage = $('#contactList').find('[data-parent2-id="' + sender_Id + '"]');
                ...
                if (receiver_Id == tutorId && sender_Id == receiverId) {
            ...
            // Fetch messages
            $.ajax({
                url: '{{ route("tutor.fetch.Messages") }}',
                method: 'POST',
                data: {
                    _token: token,
                ...
                success: function(response) {
                 //   console.log('messages:', response);
                    ...
                    response.messages.forEach(function(message) {
                        ...
                        let userAvatar = isSender ? "{{ asset(session('tutorMain')->tutor->userProfile->profile_pic) }}" : profileImage;
        ...
        function updateActiveContact() {
            $('#contactList').find('.contact-item[parent-id="' + activeContact + '"]').addClass('active');
        };

        function searchContacts(searchQuery) {
            $.ajax({
                type: 'GET',
                url: "{{ route('tutor.search.contacts') }}",
                data: {
                    query: searchQuery
                },
                beforeSend: function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            let userAvatar = "{{ asset(session('tutorMain')->tutor->userProfile->profile_pic) }}";
