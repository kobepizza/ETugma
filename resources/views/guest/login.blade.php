...
<body>
    <main class="login container growable-flex px-4 py-5">
        <div class="container col-sm-10 col-md-8 col-lg-5 col-xl-5 col-xxl-4 py-3 mb-3 border border-1 shadow-lg" style="border-radius:30px;">
            <form action="{{route('login')}}" method="POST">
                @csrf
                @if(Session::has('success'))
                <div class="alert alert-success session-alert">{{Session::get('success')}}</div>
                @endif
                @if(Session::has('info'))
                ...
                <div class="container">
                    <div class="logo text-center">
                        ...
                        <h2 class="display-7 fw-normal pb-3">Login</h2>
                    </div>
                    <div class="form-group">
                        <div class="input-group flex-nowrap mb-3">
                            ...
                            <input type="text" name="email" value="{{old('email',Cookie::get('remember_email'))}}" class="form-control" placeholder="Email Address" required>
                        </div>
                        @error('email')
                        <div class="alert alert-danger">{{ $error }}</div>
                        @enderror
                        <div class="input-group flex-nowrap mb-3 password-container">
                            <span class="input-group-text" id="addon-wrapping"><svg class="bi" width="20" height="20">
                                    <use xlink:href="#lock" />
                                </svg>
                            </span>
                            <input type="password" name="password" value="{{Cookie::get('remember_password')}}" class="form-control border-end-0" placeholder="Password" id="password-input" required>
                        ...
                        <div class="row row-cols-2 mb-3">
                            <div class="col-6 text-start">
                                <div class="form-check mb-3">
                                    <input class="form-check-input border-1 border-black" name="rememberCookie" type="checkbox" {{ Cookie::get('remember_email') ? 'checked' : '' }} id="remember">
                        ...
                        <button type="submit" class="lgn btn btn-lg btn-primary w-100 mb-3">Log In</button>
                        <div class="sign 
                        iner text-center mb-5 mt-3">
                            <h7 class="fw-normal">Don't have an account? <a class="link" href="{{route('choice')}}">Create one</a></h7>
                        </div>
                    </div>
                </div>
            </form>
