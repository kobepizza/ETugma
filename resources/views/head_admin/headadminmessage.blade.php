...
<script>
    $(document).ready(function() {
        ...
        function loadContacts(callback) {
            $.ajax({
                type: 'GET',
                url: "{{ route('head.admin.load.contacts') }}",
                dataType: 'json',
                beforeSend: function() {
        ...
        $('#contactList').on('click', '.contact-item', function() {
            ...
            let channelName = 'headAdmin-' + parentId + '-admin-' + receiverId + '-chat';
            ...
            // Fetch messages
            $.ajax({
                url: '{{ route("head.admin.fetch.messages") }}',
                method: 'POST',
                data: {
                    _token: token,
        ...
        function searchContacts(searchQuery) {
            $.ajax({
                type: 'GET',
                url: "{{ route('head.admin.search.contacts') }}",
                data: {
                    query: searchQuery
                },
                beforeSend: function() {
    ...
    $(document).ready(function() {
        ...
        $('#messageForm').on('submit', function(e) {
            ...
            if (validMessage) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("head.admin.send.Message") }}',
                    data: {
                        _token: token,
                        message: message,
